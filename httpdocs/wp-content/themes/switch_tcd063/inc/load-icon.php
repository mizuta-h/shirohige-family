...
?>
    <?php  if(($dp_options['load_first'] === 0 || ($dp_options['load_first'] === 1 && !isset($_COOKIE['first_visit']))) ){ ?>
...
<?php if ( 'type2' === $dp_options['header_content_type'] && wp_is_mobile() ){ ?>
      var hSize = window.innerHeight - 60;
      $('#js-header-video').height(hSize);
...
<?php }elseif ( 'type3' === $dp_options['header_content_type'] && wp_is_mobile() ){ ?>
      var hSize = window.innerHeight - 60;
      $('#js-header-youtube').height(hSize);
      var windowWidth = $(window).width();
      $(window).bind("resize orientationchange", function() {
        if ($(window).width() != windowWidth) {
          var hSize = window.innerHeight -60;
